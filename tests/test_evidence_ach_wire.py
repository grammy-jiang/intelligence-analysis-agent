...
from mcp_servers.ach_engine.server import mcp as ach_mcp
from mcp_servers.evidence_ledger.server import mcp as ev_mcp
...
async def _ach_hypothesis_length_cap() -> None:
    async with Client(ach_mcp) as c:
        ...
        assert "at most 10000 characters" in await err(
            {"case_id": "c", "hypotheses": ["ok", "x" * 10_001]}
        )
