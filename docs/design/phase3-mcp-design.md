...
```
Step 2 Hypotheses      → ach-engine.create_matrix(case_id, hypotheses[])        → hypothesis_ids
Step 3 Collect+grade   → evidence-ledger.add_evidence(case_id, ..., expected_observables keyed by hypothesis_id)
                       → evidence-ledger.grade_evidence(evidence_id, A–F, 1–6, judgment_source)
Step 4 ACH matrix      → ach-engine.rate_cell(matrix_id, evidence_id, hypothesis_id, C/I/N-A, judgment_source)
Step 6 Disprove/rank   → ach-engine.score_matrix(matrix_id)  # refuses if any referenced cell is stale
Step 9 Judgment        → calibration-tracker.log_forecast(case_id, ..., judgment_source="analyst_confirmed")
(later, months)        → calibration-tracker.resolve_forecast(forecast_id, outcome)
```
